...
| pH | `argisense_ph_u575rg` | `docs/ph-firmware-overview.md` | pH front-end, pH compensation, pH-specific DAC mapping |
...
The pH board has its own firmware profile and documentation. See
`docs/ph-firmware-overview.md` and `docs/sensor-platform-v1-ph-analysis.md`.
...
| Methane sensor | `uart4` | `PC10`, `PC11` | Dynament Platinum methane UART, 38400 baud; updated schematic must route `METH_UART_TXD/RXD` to a valid UART pin pair |
...
Hardware protocol code lives in out-of-tree Zephyr drivers. Shared firmware
services stay in `app/common/src`, while methane + pressure product policy,
persistent settings, Modbus registers, and shell commands stay in
`app/products/methane_pressure/src`.

## System Illustrations

![Firmware architecture with sensor inputs, power manager, RS485 service, and dual GP8302 current-loop outputs.](figures/methane-pressure-firmware-architecture.png)

![Measurement cycle showing rail enable, settle delay, sensor reads, validation, output update, rail shutdown, and Zephyr PM idle.](figures/methane-pressure-measurement-cycle.png)

![Output and communication mapping showing methane and pressure values feeding RS485 registers and two GP8302 4-20 mA outputs.](figures/methane-pressure-output-communication.png)

Current split:

| Component | Location | Responsibility |
| --- | --- | --- |
| Main loop | `app/products/methane_pressure/src/main.c` | Coordinates power, sensor reads, DAC updates, register snapshots, and MCUboot confirmation |
| Power manager | `app/products/methane_pressure/src/argisense_power.c` | Controls sensor rails, DAC power, RS485 termination, and pressure protocol-select GPIOs |
| Settings | `app/products/methane_pressure/src/argisense_settings.c` | Loads and saves runtime configuration through Zephyr Settings/NVS |
| Register model | `app/products/methane_pressure/src/argisense_registers.c` | Owns the Modbus holding-register map and coherent measurement snapshots |
| RS485 service | `app/products/methane_pressure/src/argisense_rs485.c` | Starts the Zephyr Modbus RTU server and forwards reads/writes to the register model |
| USB-C service | `app/common/src/argisense_usb.c`, `snippets/argisense-usb-update` | Owns the composite CDC ACM USB device profile for console, shell, and MCUmgr firmware update |
| Shell diagnostics | `app/products/methane_pressure/src/argisense_shell.c` | Provides driver, sensor, settings, and register inspection commands |
| Current-loop output | `app/products/methane_pressure/src/current_loop_output.c` | Converts methane and pressure process values to GP8302 current commands |
...
```text
app/common/src/argisense_dfu.c
app/common/src/argisense_dfu.h
app/common/src/argisense_usb.c
app/products/methane_pressure/src/main.c
app/products/methane_pressure/src/argisense_power.c
app/products/methane_pressure/src/argisense_power.h
app/products/methane_pressure/src/argisense_settings.c
app/products/methane_pressure/src/argisense_settings.h
app/products/methane_pressure/src/argisense_registers.c
app/products/methane_pressure/src/argisense_registers.h
app/products/methane_pressure/src/argisense_rs485.c
app/products/methane_pressure/src/argisense_rs485.h
app/products/methane_pressure/src/argisense_shell.c
app/products/methane_pressure/src/current_loop_output.c
app/products/methane_pressure/src/current_loop_output.h
...
- Out-of-tree Zephyr board targets for methane + pressure and pH hardware
  profiles.
- Placeholder devicetree bindings for the future pH ADS124S08 ADC, AD5675
  I2C bias DAC, and LT3582 analog rail controller.
...
- Confirm the pH MCU pin table before enabling ADS124S08, AD5675, LT3582,
  PT1000, and electrode-zero firmware.
